# Architecture Decision Records (ADRs)

This directory contains records of important architectural decisions made for the Impulsa project.

## List of ADRs

| ID                             | Title                       | Status   | Date       |
| :----------------------------- | :-------------------------- | :------- | :--------- |
| [001](001-AUTH-STRATEGY.md)    | **Authentication Strategy** | Accepted | 2025-11-25 |
| [002](002-OFFLINE-STRATEGY.md) | **Offline-First Strategy**  | Accepted | 2025-11-26 |
| [003](003-ICON-SYSTEM.md)      | **Icon System (Lucide)**    | Accepted | 2026-01-27 |

## Process

When creating a new ADR:

1. Copy `docs/templates/02-ADR-TEMPLATE.md`.
2. Name it `NNN-TITLE-SLUG.md` (incrementing number).
3. Fill in the required sections.
4. Update this index.
