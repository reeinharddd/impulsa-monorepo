...
### 9.9. File Permission Issues (Linux)

**Problem:** Files generated by Docker (e.g., `index.ts`, `node_modules`) are owned by `root`, causing `EACCES` errors when trying to save or format them on the host.

**Why this happens:** Docker containers often run as root by default. If a container writes to a bind-mounted volume (like generating assets), the files on the host will be owned by root.

**Solution:**

1. **Fix existing permissions:**
   We provide a utility script to fix this instantly:

   ```bash
   bun run permissions:fix
   # Or manually:
   sudo chown -R $USER:$USER .
   ```

2. **Prevention:**
   - Avoid running file-generation commands (like `bun run build`) _inside_ the container for development if possible. Run them on the host.
   - If running inside Docker, ensure the container runs as your user:
     ```yaml
     services:
       web:
         user: "${UID}:${GID}"
     ```
     _(Note: This requires exporting UID/GID environment variables)_
