...
@Component({
  ...
  template: `
    <div class="active-charge-container" *ngIf="currentIntent(); else loading">
      <div class="status-card">
        ...
        <div class="status-badge" [class]="currentIntent()!.status">{{ currentIntent()!.status }}</div>
        ...
        <div class="mock-controls">
            <h4>[MOCK CONTROLS]</h4>
            <button (click)="simulatePaymentSent()" [disabled]="currentIntent()!.status !== 'CREATED' && currentIntent()!.status !== 'PRESENTED'">
  ...
  styles: [`
    ...
    .amount { font-size: 3rem; margin: 0; color: #111827; }
    .status-badge {
    ...
    .status-badge.CREATED { background: #eff6ff; color: #1e40af; }
    .status-badge.PRESENTED { background: #e0f2fe; color: #0369a1; }
    .status-badge.PAYMENT_SENT { background: #fef9c3; color: #854d0e; }
    .status-badge.CONFIRMED { background: #dcfce7; color: #166534; }
    .status-badge.CANCELED { background: #fef2f2; color: #ef4444; }

    .reference { color: #6b7280; font-family: monospace; letter-spacing: 1px; }

    .channels-section { margin-top: 2rem; }
    .channels-grid { display: flex; justify-content: space-around; margin: 1.5rem 0; }
    .channel-card { display: flex; flex-direction: column; align-items: center; gap: 8px; cursor: pointer; }
    .icon {
        width: 50px; height: 50px; background: #f3f4f6; border-radius: 50%;
        display: flex; justify-content: center; align-items: center; font-weight: 700; font-size: 0.8rem;
    }
    .icon.qr { background: #dbeafe; color: #1e40af; }
    .icon.nfc { background: #fae8ff; color: #86198f; }
    .icon.link { background: #fce7f3; color: #9d174d; }
    ...
    .btn-cancel { background: #fee2e2; border: 1px solid #ef4444; color: #b91c1c; border-radius: 4px; }
  `]
})
