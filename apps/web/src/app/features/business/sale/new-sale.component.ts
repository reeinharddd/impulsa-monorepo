...
@Component({
        ...
        <div class="grid">
            @for (product of products(); track product.id) {
                <button class="product-card"
                    ...
                    <div class="p-stock" [class.empty]="product.stock <= 0">Stock: {{ product.stock }}</div>
                </button>
            }
        </div>
      </div>
        ...
        <div class="cart-items">
            ...
            @for (item of cart(); track item.product.id) {
                <div class="cart-item">
                    ...
                    <span class="item-total">{{ (item.product.priceCents * item.quantity) / 100 | currency }}</span>
                </div>
            }
        </div>
  ...
  styles: [`
    ...
    .p-name { font-weight: 600; margin-bottom: 5px; }
    .p-price { color: #2563eb; font-weight: 700; }
    .p-stock { font-size: 0.8rem; color: #6b7280; margin-top: 5px; }
    ...
    .item-controls button {
        background: #e5e7eb; border: none; width: 24px; height: 24px; border-radius: 4px; cursor: pointer;
    }
    .item-total { font-weight: 600; font-size: 0.9rem; }

    .cart-footer { margin-top: 1rem; border-top: 2px solid #e5e7eb; padding-top: 1rem; }
    .total-row { display: flex; justify-content: space-between; font-size: 1.2rem; font-weight: 700; margin-bottom: 1rem; }
    .btn-charge {
        width: 100%;
        background: #111827;
        color: white;
